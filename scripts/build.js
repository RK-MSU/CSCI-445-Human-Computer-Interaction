...
module.exports = () => {
	...
	let themeTemplate = CONFIG.pageTemplatePath
	...
	.pipe(Through2.obj((file, _, cb) => {
		...
		let assets_dir_path = CONFIG.assetsOutputDir;
		assets_dir_path = assets_dir_path.replace(CONFIG.outputDir, "");
		assets_dir_path = Path.join(relPath, assets_dir_path) + "/";
		assets_dir_path = (assets_dir_path[0] == '/') ? assets_dir_path.substring(1) : assets_dir_path;
...
function getMasterToc() {
	...
	try {
		 toc = Yaml.load(Fs.readFileSync(CONFIG.tocPath, 'utf8'))
	} catch (e) {
